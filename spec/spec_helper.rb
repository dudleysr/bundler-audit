...
module Helpers
  ...
  def expect_update_to_clone_repo!
    expect(Bundler::Audit::Database).
      to receive(:system).
      with('git', 'clone', Bundler::Audit::Database::VENDORED_PATH, mocked_default_path).
      and_call_original
  end
  ...
  def fake_a_commit_in_the_user_repo
    Dir.chdir(mocked_default_path) do
...
RSpec.configure do |config|
  ...
  config.before(:each) do
    ...
    stub_const("Bundler::Audit::Database::DEFAULT_PATH", mocked_default_path)
    FileUtils.rm_rf(mocked_default_path) if File.exist?(mocked_default_path)
  end
end
